...
void TM_NRF24L01_SendData(uint8_t *buff){
    TM_NRF24L01_Transmit_Status_t transmissionStatus; 
    
    TM_NRF24L01_Transmit(buff);          
    do {        // Wait for data to be sent
          transmissionStatus = TM_NRF24L01_GetTransmissionStatus();
    } while (transmissionStatus == TM_NRF24L01_Transmit_Status_Sending);
   
   //Необходимо Без этого не переходит в режим приема
   for(int i = 0; i < 2; ++i) __no_operation();
...
uint8_t TM_NRF24L01_Init(uint8_t channel, uint8_t payload_size) {
	...
	TM_NRF24L01_InitPins();                         //Initialize CE and CSN pins
	TM_SPI_Init(NRF24L01_SPI, NRF24L01_SPI_PINS);   //Initialize SPI
	...
	TM_NRF24L01_SetChannel(TM_NRF24L01_Struct.Channel);     //Channel select
	...
	TM_NRF24L01_PowerUpRx();                //Go to RX mode
        ...
        TM_NRF24L01_SetMyAddress(TxAddress); // Set my address, 5 bytes
