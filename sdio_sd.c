...
BYTE SD_State = (STA_NOINIT | STA_NODISK); //Жранит состояние карточки и слота для FAT_FS
...
{ 
  SD_LowLevel_DeInit();
  SD_State |= (STA_NODISK | STA_NOINIT); //ставим биты без диска и не инициализировано
}
...
SD_Error SD_Init(void)
{
  ...
  if (errorstatus == SD_OK) {
    SD_State &= ~STA_NODISK;
    SD_State &= ~STA_NOINIT; //снимаем биты без диска и не инициализировано
  }
...
/**
  * @brief  Gets the curent sd card data transfer status.
  * @param  None
...
//В стандартном примере эта часть кода расположена ниже. И поэтому происходят сбои чтения - проц подвисает. Читай ссылки ниже.
//http://forum.easyelectronics.ru/viewtopic.php?f=35&t=12831
//https://my.st.com/public/STe2ecommunities/mcu/Lists/cortex_mx_stm32/Flat.aspx?RootFolder=/public/STe2ecommunities/mcu/Lists/cortex_mx_stm32/Timing,%20DMA%20%20SDIO%20Bug%20in%20Standards%20Peripherals%20Examples%20V3.5.0&FolderCTID=0x01200200770978C69A1141439FE559EB459D7580009C4E14902C3CDE46A77F0FFD06506F5B&currentviews=161
